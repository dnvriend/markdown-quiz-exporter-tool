# markdown-quiz-exporter - Project Specification

## Goal

A CLI tool that exports the quiz-markdown-format to tools like Anki, Flashcard Hero

## What is markdown-quiz-exporter?

`markdown-quiz-exporter` is a command-line utility built with modern Python tooling and best practices.
...
```bash
markdown-quiz-exporter [OPTIONS]
```
...
3. **Usage Pattern** - User-friendly command
   ...
   markdown-quiz-exporter completion bash
   markdown-quiz-exporter completion zsh
   markdown-quiz-exporter completion fish
...
5. **Installation Methods**
   - **Temporary**: `eval "$(markdown-quiz-exporter completion bash)"`
...
1. Create new command module in `markdown_quiz_exporter/`
2. Import and add to CLI group:
   ```python
   from markdown_quiz_exporter.new_command import new_command
...
```bash
cd /path/to/markdown-quiz-exporter
mise use -g python@3.14
uv sync
uv tool install .
```

After installation, `markdown-quiz-exporter` command is available globally.
